(() => {
    ...
    let timerId, queue, initialQueue, pixelsArray = [];
    ...
    function showCadre() {
        const cadre = queue.slice(0, length);
        for (let i = 0; i < length; i++) {
            for (let j = 0; j < cadre[i].length; j++) {
                const pixelClassList = pixelsArray[i * cadre[i].length + j].classList;
                if (cadre[i][j] === '1') {
                    if (!pixelClassList.contains('alight-pixel')) {
                        pixelClassList.add('alight-pixel');
                    }
                } else {
                    if (pixelClassList.contains('alight-pixel')) {
                        pixelClassList.remove('alight-pixel');
                    }
                }
            }
        }
